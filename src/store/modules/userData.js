  ...
  actions: {
    ...
    ADD_USER_API_TOKEN({ commit, getters, dispatch }, payload) {
      ...
      try {
        firebase
          .database()
          .ref("userData/" + getters.userId + "/apiToken/" + payload.index)
          .update({
            name: payload.name,
            key: payload.key,
    ...
    DELETE_API_TOKEN({ commit, getters }, payload) {
      ...
      if (payload != Object.keys(getters.apiToken).length) {
        ...
        firebase
          .database()
          .ref("userData/" + getters.userId + "/apiToken")
          .remove();
        firebase
          .database()
          .ref("userData/" + getters.userId + "/apiToken")
        ...
        try {
          firebase
            .database()
            .ref("userData/" + getters.userId + "/apiToken/" + payload)
            .remove();
        } catch (e) {
          commit("SET_ERROR", e);
          commit("SET_PROCESSING", false);
    ...
    LOAD_ORDERS({ commit, getters }) {
      Vue.$cdb
        .collection(`wbData/${getters.userId}/orders`)
        .get()
        .then((querySnapshot) => {
          let orders = [];
