  ...
  actions: {
    ADD_USER_CAMPAING({ commit, getters, dispatch }, payload) {
      ...
      try {
        firebase
          .database()
          .ref("userData/" + getters.userId + "/campaings/" + payload.index)
          .update({
            name: payload.name,
            balance: 0,
            status: "Идут показы",
    ...
    COMPLETE_CAMPAING({ commit, getters }, payload) {
      ...
      try {
        firebase
          .database()
          .ref("userData/" + getters.userId + "/campaings/" + payload)
          .update({
    ...
    LOAD_CAMPAINGS({ commit, getters }) {
      var userDataRef = firebase
        .database()
        .ref("userData/" + getters.userId + "/campaings");
      try {
    ...
    LOAD_BRANDS({ commit, getters }) {
      Vue.$cdb
        .collection(`wbData/${getters.userId}/brands`)
        .get()
        .then((querySnapshot) => {
          let brands = [];
